...
eslint 를 command line 에서 사용할 때 watch 모드를 실행하고 싶었다.  
...
eslint 가 중복 설치된 것은 해결했고 이제 실행을 했다. (실행 script 는 문서를 참조했다.)  
그런데 watch 모드가 실행되지 않고 내가 수동으로 lint 를 매번 실행시켜줘야했다..

다시 한번 [eslint-watch 문서](https://www.npmjs.com/package/eslint-watch#requirements) 를 살펴보니 `"eslint": ">=7 <8.0.0"` 버전에서 사용 가능하다고 한다. 내 CRA 는 확인해보니 6.6 버전에 의존하고 있다.
